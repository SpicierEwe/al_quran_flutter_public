              ...
              Html(
                ...
                style: {
                  "body": Style(
                    ...
                    direction: Utils.translationTextDirection(context: context),
...
// displays the single verse verse by verse format
  static Widget displaySingleVerseWithTranslation(
      {required SettingsState settingsState,
      // required List<dynamic> surahData,
      required int verseIndex,
      required BuildContext context,
      required SurahTrackerState surahTrackerState,
    ...
    final String verseTranslation =
        LocalDataRepository.getStoredQuranChapterTranslation(
                chapterId: surahIndex + 1,
                translationId: settingsState.selectedTranslationId)![verseIndex]
            ["text"];
    return Column(
      children: [
        Wrap(
          ...
          textDirection: TextDirection.rtl,
          spacing: Utils.wordSpacingSettings(settingsState: settingsState),
          children: [
            for (int wordIndex = 0;
                wordIndex < surahData[verseIndex]["words"].length;
                wordIndex++)
              // TOOL TIP
              Tooltip(
                  decoration: BoxDecoration(
                  ...
                  onTriggered: () async {
                    ...
                    context.read<SurahTrackerBloc>().add(
                        UpdateHighlightWordEvent(
                            wordLocation: surahData[verseIndex]["words"]
                                [wordIndex]["location"]));
                  ...
                  message: surahData[verseIndex]["words"][wordIndex]
                      ["translation"]["text"],
                  child: settingsState.selectedQuranScriptType == "tajweed"
                      ? Container(
                          ...
                          color: Utils.highlightTajweedWordImage(
                              context: context,
                              showRecitationWordHighlight: false,
                              quranDisplayType: QuranDisplayType.surah,
                              audioPlayerQuranDisplayType: context
                              ...
                              currentWordLocation: surahData[verseIndex]
  ...
  static int? bookmarkedMushafPageIndex(
      {bool isJuz = false,
      required List pages,
      int? surahIndex,
      int? juzIndex,
    ...
    if (context.read<BookmarkBloc>().state.lastRead["page_index"] != null) {
      final bookmarkedJuzIndex =
          context.read<BookmarkBloc>().state.lastRead["juz_index"];
      final bookmarkedSurahIndex =
          context.read<BookmarkBloc>().state.lastRead["surah_index"];
      ...
      * related to the actual page index of the quran*/
      final bookmarkedPageIndex = pages.indexOf(
          context.read<BookmarkBloc>().state.lastRead["page_index"] + 1);

      // if is juz
      if (isJuz && juzIndex == bookmarkedJuzIndex) {
        return bookmarkedPageIndex;
      }

      // if is surah
      if (bookmarkedSurahIndex == surahIndex) {
        return bookmarkedPageIndex;
      }

      Logger().i("page_index  : " +
          context.read<BookmarkBloc>().state.lastRead["page_index"].toString());
